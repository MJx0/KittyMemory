...
MemoryPatch MemoryPatch::createWithHex(const ProcMap &map, uintptr_t address, const std::string &hex)
{
  MemoryPatch patch;

  std::string tmp_hex = hex;
  if (!map.isValid() || address == 0 || !KittyUtils::validateHexString(tmp_hex))
    return patch;
