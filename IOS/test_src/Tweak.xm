...
void *HackThread(void *user_data) {
    ...
    memory_file_info some_bin = KittyMemory::getMemoryFileInfo("xxxxx");

    uintptr_t found_at = KittyScanner::find_from_segment64(some_bin.header, "__TEXT", "\x00\x00\x00\x00\x00\x00\x00\x00\x00", "xxxx??x?x");
    NSLog(@"scanner found bytes at: %p", (void*)found_at);
